...
class Cliente:
    ...
    def start(self):
        ...
        available_chunks = dict()
        while set(list(available_chunks.keys())) != set(self.list_chunks):
            ...
            if msg_type == 3:
                ...
                for i in range(num_chunks):
                    ...
                    if chunk_id not in list(available_chunks.keys()):
                        available_chunks[chunk_id] = addr
        
            print('available_chunks= ', set(list(available_chunks.keys())))
        
        for addr_peer in set(available_chunks.values()):
            list_chunks_availables = [k for k,v in available_chunks.items() if v == addr_peer]
        ...
        while True:
            ...
            with open(f'output/new_chunk_{chunk_id}', 'wb') as f:
                f.write(payload)
            print(f'file output/new_chunk_{chunk_id} writed')
